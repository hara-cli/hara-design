...
(function () {
  ...
  // messages loop back into our own message listener and fight local view changes (the exported-file Grid↔Play bug).
  ...
  // Grid/Play toggle so the exported single file is fully usable. In the preview, parent !== window so the host owns it.
  if (screens.length > 1 && window.parent === window) {
    var eseg = mk("div", "hara-export-toggle");
    ["grid", "detail"].forEach(function (v) { var b = mk("button", ""); b.dataset.v = v; b.textContent = v === "grid" ? "▦ Grid" : "▶ Play"; eseg.appendChild(b); });
